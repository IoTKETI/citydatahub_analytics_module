...
class PreprocessedDataView(APIView):
    def post(self, request):
        ...
        if query.exists():
            get_pk_new = (
                PreprocessedData.objects.latest("PREPROCESSED_DATA_SEQUENCE_PK").pk + 1
            )
        else:
            get_pk_new = 1
    ...
    def get(self, request):
        queryset = PreprocessedData.objects.all().order_by(
    ...
    def delete(self, request, pk):
        ...
        if serializer.data["DELETE_FLAG"]:
            ...
            if os.path.isfile(serializer.data["FILEPATH"]):
                ...
                return Response(
                    error_code.FILE_NOT_FOUND_4004(
...
def pfunction_download(request, pk):
    serializer = PreprocessedDataSerializer(get_object_or_404(PreprocessedData, pk=pk))
    if serializer.data["DELETE_FLAG"]:
        return Response(
            error_code.CONFLICT_4009(mode="DELETE", error_msg="deleted"),
            status=status.HTTP_409_CONFLICT,
