    ...
    def post(self, request):
        ...
        if isinstance(check_result, dict):  # check_result 타입이 dict 이면 에러 메시지를 반환한 것!
            ...
            if error_type == "4004":
                ...
                return Response(
                    error_code.UNPROCESSABLE_ENTITY_4022(error_msg),
                    status=status.HTTP_422_UNPROCESSABLE_ENTITY,
                )
            elif error_type == "4103":
                return Response(
                    error_code.INVALID_MODEL_PARAMETER_4103(error_msg),
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class TrainModelDetailView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if not check_result:  # check_result 가 True 아니면 에러 메시지를 반환한 것!
            error_type = check_result["error_type"]
            error_msg = check_result["error_msg"]
            if error_type == "4101":
                return Response(
        ...
        # 수정하는 경우 2 = 모델 재시작
            ...
            P_serializer = PreprocessedDataSerializer(
                PreprocessedData.objects.get(pk=P_pk)
            ).data
        ...
        # 데이터 전달 및 데이터 적용 테스트
            ...
            request_data_path = os.path.join(
                base_directory, user_request["test_data_path"]
            )
            ...
            train_y = ast.literal_eval(serializer.data["COMMAND"])["train_parameters"][
                "y"
            ]
            ...
            test_result = test_result.get_test_result(target=test_result.y_data)
    ...
    def delete(self, request, pk):
        # DELETE_FLAG == True 전환하고, 저장된 모델 삭제 (db의 instance 삭제하지 않음)
        ...
        if serializer.data["DELETE_FLAG"]:
            ...
            if os.path.isfile(serializer.data["FILEPATH"]):
                ...
                return Response(
                    error_code.FILE_NOT_FOUND_4004(path_info=serializer.data["FILEPATH"]),
                    status=status.HTTP_404_NOT_FOUND,
                )
...
def model_download(request, pk):
    serializer = TrainModelSerializer(get_object_or_404(TrainInfo, pk=pk))
    if serializer.data["DELETE_FLAG"]:
        return Response(
            error_code.CONFLICT_4009(mode="DELETE", error_msg="deleted"),
            status=status.HTTP_409_CONFLICT,
